...
const Icon = ({ name, x, y, imgSrc, positions, updatePosition, onDoubleClick }) => {
    ...
    return (
        <Draggable
            ...
            disabled={!isDragging}
        >
            <div className="flex flex-col items-center absolute" onDoubleClick={() => onDoubleClick(name)}>
                <div className="w-24 h-24 flex items-center justify-center rounded-lg shadow-sm cursor-pointer transition-transform duration-150 bg-green-200 overflow-hidden">
                    {imgSrc && <img src={imgSrc} alt={`${name} icon`} className="w-full h-full object-contain" draggable="false"/>}
                </div>
